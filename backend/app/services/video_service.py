...
class VideoService:
    ...
    def __init__(self,
        ...
        if shelf_regions is None:
            shelf_regions = {
                'shelf_a': (100, 200, 500, 600),
                'shelf_b': (600, 200, 1000, 600)
            }
    ...
    def process_frame(self, frame: np.ndarray, visualize: bool = True) -> Dict:
        ...
        # In synthetic mode, we might skip actual heavy detection to save resources,
        # OR we can let it run on the synthetic image (YOLO might not find much).
        # For demo purposes, we'll just return an empty detection list or basic mock data if synthetic.
        ...
        if not self.synthetic_mode:
            ...
            # Mock detections for the demo ball
            # We know exactly where we drew the ball
            t = time.time()
            x = int(600 + 400 * abs(np.sin(t / 2)))
            y = 400
            # Mock a detection object just for visualization
            detections = [] 
            # In a real synthetic test we would construct a Proper detection object.
            # But for now, let's just let the frame serve as the visual.
            pass
    ...
    def read_frame(self):
        ...
        if self.synthetic_mode:
            ...
            if self.cap:
                ret, frame = self.cap.read()
                if ret and frame is not None:
                    # Resize if larger than 640 width to ensure performance
                    if frame.shape[1] > 640:
                        scale = 640 / frame.shape[1]
                        new_height = int(frame.shape[0] * scale)
                        frame = cv2.resize(frame, (640, new_height))
